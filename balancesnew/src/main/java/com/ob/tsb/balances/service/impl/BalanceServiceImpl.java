...
public class BalanceServiceImpl implements BalanceService {
   ...
   // @CircuitBreaker(name = "balanceService", fallbackMethod = "balanceServiceCbFallback")
    public Mono<OBReadBalance> getBalances(String consentId) {
        try{
           Map<String, String> acc = consentService.getBalancesConsents(consentId);
            return getBalanceDetails(acc);
        }catch (ObCustomException e){
            throw e;
        }
   ...
   public Mono<OBReadBalance> getBalanceDetails(Map<String, String> authorizedAccounts) {
            ...
            if(Objects.isNull(obBalance)){
             ...
             return obBalance.handle((ob, sink) -> {
              Errors errors =  validationService.validateObBalance(ob);
              if(errors.hasErrors()){
                  //have to notify BIAN about validation result
                  sink.error(new ValidationException(ValidationConstraints.ERROR_OB_BALANCE_VALIDATION)); //ResponseEntity.ok().body(ValidationConstraints.ERROR_OB_BALANCE_VALIDATION);
              } else {
                  sink.next(new OBReadBalance(new OBReadBalance.Data(ob), new OBReadBalance.Links(OB_SPECIFICATION_URL), new OBReadBalance.Meta(1)));
