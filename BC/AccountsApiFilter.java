...
import static com.ob.tsb.accounts.util.ApplicationConstants.ACCESS_FORBIDDEN;


@Component
@Order(1)
public class AccountsApiFilter implements WebFilter{
    @Autowired
    ...
    private static final String SWAGGER_RESOURCES_PATH = "/webjars/swagger-ui/";

    @Override
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        String path = exchange.getRequest().getPath().toString();

        if (path.contains(SWAGGER_UI_PATH)) {
